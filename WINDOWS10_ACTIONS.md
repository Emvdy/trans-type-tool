...
指定文本文件或读取本地 Windows 剪贴板。`--source` 可以直接接路径，旧的
`--file <路径>` 仅作为兼容别名保留：
...
```powershell
.\trans_type.exe --mode cmd-hex --source c:\work\input.txt --remote-output trans.txt
```
...
```powershell
.\trans_type.exe --mode zip-hex --source c:\work\input.txt --remote-output trans.txt
```
...
```powershell
.\trans_type.exe --mode cmd-hex --source c:\work\tool.exe --output-encoding preserve --remote-output tool.exe
.\trans_type.exe --mode zip-hex --source c:\work\data --remote-output copied-dir
```

目录来源只允许 `zip-hex`。`copied-dir` 是远端目标目录；解压会覆盖/合并同名文件，
...
CRC 会继续校验每个成员。远端 `tt.hex` 和 `tt.zip` 会保留用于恢复；确认输出正确
后再人工删除。
